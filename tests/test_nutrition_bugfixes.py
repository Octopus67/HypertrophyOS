...
def test_update_entry_rejects_negative_micro():
    ...
    with pytest.raises(ValueError, match="micro_nutrient value must be >= 0"):
        NutritionEntryUpdate(
            micro_nutrients={"vitamin_c_mg": -10}
        )
...
async def test_copy_preserves_source_meal_id(db_session):
    ...
    # Create test user
    test_user = User(id=uuid.uuid4(), email="test@example.com", hashed_password="x", auth_provider="email", role="user")
...
async def test_start_date_only_returns_entries_through_today(db_session):
    ...
    # Create test user
    test_user = User(id=uuid.uuid4(), email="test2@example.com", hashed_password="x", auth_provider="email", role="user")
    db_session.add(test_user)
    await db_session.flush()
